...
## ⚔️ Bash Battle Arena – Levels 1–12

| Level | Script | Description |
|:-----:|:--------|:-------------|
| 01 | `01_basics.sh` | Creates a directory called *Arena*, adds text files, and lists them. |
| 02 | `02_numbers_1_to_10.sh` | Prints the numbers 1 through 10 using a loop. |
| 03 | `03_conditional_statement.sh` | Checks if `hero.txt` exists in the Arena directory. |
| 04 | `04_backup_script.sh` | Copies all `.txt` files from *Arena* into a `Backup` directory. |
| 05 | `05_combining_basics.sh` | Combines directory creation, file operations, and conditionals. |
| 06 | `06_number_of_lines.sh` | Counts and prints the number of lines in a given file. |
| 07 | `07_sort_files.sh` | Sorts `.txt` files in a directory by size (smallest → largest). |
| 08 | `08_search_logs.sh` | Searches all `.log` files for a specific word or phrase. |
...
| Script | Description |
|--------|-------------|
| `basic_arithmetic.sh` | Takes two numbers and performs addition, subtraction, multiplication, and division. |
| `file_operations.sh` | Creates a directory, file, writes text, and displays the content. |
